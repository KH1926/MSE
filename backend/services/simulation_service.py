...
try:
    ...
    from services.kpi_service import compute_and_store_kpi
    from services.schedule_service import get_station_params
        ...
        while cursor < len(task_queue):
            ...
            if (
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (work_order_no, station_id, EVENT_FAULT_START, fault_start, "故障注入"),
                )
                conn.execute(
                    """
                    INSERT INTO production_event (
                    ...
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (work_order_no, station_id, EVENT_FAULT_END, fault_end, "故障恢复"),
                )
                events_created += 2
            ...
            if (
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (work_order_no, station_id, EVENT_RUSH_ORDER, rush_time, "插单扰动事件"),
                )
                events_created += 1
                rush_logged = True
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (work_order_no, station_id, EVENT_START, actual_start, None),
            )
            conn.execute(
                """
                INSERT INTO production_event (
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (work_order_no, station_id, EVENT_COMPLETE, actual_end, None),
            )
            events_created += 2
...
def run_simulation(payload: SimulationRequest) -> dict[str, Any]:
    ...
    return {**strategy_result, "kpi_snapshot": kpi}
...
def list_quality_records(limit: int) -> list[dict[str, Any]]:
    with get_connection() as conn:
        return query_all(
            conn,
            """
