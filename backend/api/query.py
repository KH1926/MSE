...
try:
    ...
    from services.kpi_service import get_kpi_compare, get_latest_kpi
...
@router.get("/events")
def get_events_route(limit: int = Query(default=100, ge=1, le=1000)) -> dict[str, object]:
...
def get_quality_records_route(
    ...
    return ok(list_quality_records(limit))


@router.get("/kpi/latest")
def get_kpi_latest_route() -> dict[str, object]:
    return ok(get_latest_kpi())
